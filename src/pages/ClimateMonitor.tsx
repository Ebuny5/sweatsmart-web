  ...
  return (
    <div className="bg-gray-900/70 border border-gray-600 rounded-xl p-4 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
        <span className="text-gray-400">Location permission:</span>
        ...
        <span className="text-gray-400">Last log time:</span>
        <span className="text-white">{fmt(lastLogTime)}</span>
        <span className="text-gray-400">Next log time:</span>
        <span className="text-white">{fmt(nextLogTime)}</span>
        <span className="text-gray-400">Last weather fetch:</span>
...
const ClimateMonitor = () => {
  ...
  // Persist lastAlertType + timestamp across reloads
  const [lastAlertType, setLastAlertType] = useState<string | null>(() =>
    localStorage.getItem('climateLastAlertType')
  );

  // lastLogTime persisted to localStorage as anchor for 4-hour reminder
  ...
  // ============================================================
  // ALERT LOGIC — REAL DATA ONLY, EDA GATED TO WEARABLE+FRESH
  // ============================================================
  useEffect(() => {
    ...
    // Gate EDA: only wearable+fresh EDA contributes to risk score
    ...
    // Persist to survive page reload
...
// Then continue with the useEffect calls below
  ...
  const handleLogSubmit = (level: HDSSLevel) => {
    ...
    // Persist lastLogTime and anchor next reminder to it
