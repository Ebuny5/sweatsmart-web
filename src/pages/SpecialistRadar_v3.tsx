...
const SCOPE_RADII: Record<ScopeFilter, number> = {
  city:      25000,   // 25km — practical for large African cities
  country:   300000,
  continent: 5000000,
};

// ── Haversine for movement gate ────────────────────────────────────────────
...
const SpecialistRadar = () => {
  ...
  const isFetchingRef   = useRef<boolean>(false);          // in-flight lock
  const lastFetchedRef  = useRef<{ lat: number; lng: number; scope: string } | null>(null); // movement gate
  ...
  const telehealth = useMemo(() => doctors.filter(d => d.isTelehealth).sort((a, b) => a.name.localeCompare(b.name)), [doctors]);
  ...
  // ── Fetch specialists — stable, debounced, gated ────────────────────────
  // Uses refs for location/scope/geocode data so the callback never needs
  // to be recreated, eliminating the cascading re-render loop.
  const locationRef    = useRef<{ lat: number; lng: number } | null>(null);
  const scopeRef       = useRef<ScopeFilter>('city');
  const countryCodeRef = useRef<string>('');
  const continentRef   = useRef<string>('Africa');
  ...
  const fetchDoctors = useCallback(async (force = false) => {
    ...
    // In-flight lock — never run two fetches simultaneously
    if (isFetchingRef.current && !force) return;

    // Movement gate — only re-fetch if user moved >250m OR scope changed
    const last = lastFetchedRef.current;
    if (!force && last) {
      const moved = haversineDist(loc.lat, loc.lng, last.lat, last.lng);
      if (moved < 250 && last.scope === scopeRef.current) return;
    }
    ...
    try {
      ...
      const res = await fetch(RADAR_URL, {
        ...
        body: JSON.stringify({
          ...
          radius: SCOPE_RADII[scopeRef.current],
      ...
      // Record what we fetched so the movement gate works next time
      lastFetchedRef.current = { lat: loc.lat, lng: loc.lng, scope: scopeRef.current };
    } catch (e) {
  ...
  }, [user]); // only depends on user — everything else via refs

  // Keep refs in sync with state (no extra re-renders)
  useEffect(() => { locationRef.current = location; }, [location]);
  useEffect(() => { scopeRef.current = scope; }, [scope]);
  useEffect(() => { countryCodeRef.current = countryCode; }, [countryCode]);
  useEffect(() => { continentRef.current = continent; }, [continent]);
  useEffect(() => { cityRef.current = city; }, [city]);
  useEffect(() => { stateRef.current = state; }, [state]);
  useEffect(() => { countryRef.current = country; }, [country]);

  // Trigger fetch ONCE when location first arrives, with debounce to let
  // geocode refs settle before the fetch fires
  useEffect(() => {
    if (!location) return;
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => fetchDoctors(), 800);
    return () => { if (debounceRef.current) clearTimeout(debounceRef.current); };
  }, [location, fetchDoctors]);

  // Scope change → force re-fetch immediately (user deliberately changed it)
  useEffect(() => {
    if (!location) return;
    lastFetchedRef.current = null; // reset gate so scope change always fetches
    fetchDoctors(true);
  }, [scope]); // intentionally excludes fetchDoctors — we want this to fire on scope only

  // ── Map: re-center ONLY when scope changes or first location arrives ──────
  const lastMapViewRef = useRef<{ lat: number; lng: number; scope: string } | null>(null);
  useEffect(() => {
    if (!mapInst.current || !location) return;
    const L = (window as any).L;
    const last = lastMapViewRef.current;
    const moved = last ? haversineDist(location.lat, location.lng, last.lat, last.lng) : 999;
    const scopeChanged = last?.scope !== scope;
    if (moved < 200 && !scopeChanged) return; // ignore GPS jitter < 200m
    ...
    userPin.current = L.marker([location.lat, location.lng], { icon: uIcon })
    ...
    lastMapViewRef.current = { lat: location.lat, lng: location.lng, scope };
  }, [location, scope]);

  // ── Map: update doctor pins when results change ────────────────────────
  useEffect(() => {
    ...
    physical.forEach(doc => {
      ...
      const m = L.marker([doc.lat, doc.lng], { icon })
        .addTo(mapInst.current)
