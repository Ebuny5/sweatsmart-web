...
const AIGreeting = ({ profile, hdss, meta, city, onDismiss }: {
  ...
  const name = (profile?.display_name || profile?.name)?.split(' ')[0] || 'Warrior';
...
const SpecialistRadar = () => {
  ...
  }, [location, user, scope, city, state, country, countryCode, continent]);
  ...
  useEffect(() => {
    ...
    const onLocationUpdate = async (pos: GeolocationPosition) => {
      ...
      // Ignore tiny GPS jitter — require 250m movement before triggering fetches
      ...
      try {
        const res = await fetch(
          ...
          { headers: { 'User-Agent': 'SweatSmart-App/1.0' } }
        );
        ...
        fetchDebounceRef.current = window.setTimeout(() => {
          if (!isFetchingRef.current) fetchDoctors();
        }, 800);
    ...
    const watcherId = navigator.geolocation.watchPosition(onLocationUpdate, onError, {
