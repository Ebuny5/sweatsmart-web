...
const ClimateNotificationSidebar: React.FC<ClimateNotificationSidebarProps> = ({ isOpen, onClose }) => {
    ...
    // FIX 3: Track last notification time to prevent alert spam on every weatherData state update.
    // Previously a notification fired every time weatherData changed (including simulated ticks),
    // which could mean the notification values didn't match what was shown on screen.
    const lastNotificationRef = React.useRef<number>(0);
    const NOTIFICATION_COOLDOWN = 30 * 60 * 1000; // 30 minutes between climate alerts
    ...
    // Fetch weather immediately when location is obtained
    useEffect(() => {
        if (location) {
    ...
    // FIX 1: Periodically re-fetch real weather every 10 minutes while app is open.
    // Previously the app ONLY fetched on location change, so weather went stale immediately.
    ...
    useEffect(() => {
        // FIX 2: Only simulate weather when permissions are NOT yet granted AND there's no real
        // location available. Previously this ran even after real data was fetched, which caused
        // the simulated values to diverge from real values and trigger wrong notifications.
    ...
    useEffect(() => {
        ...
        // Update the on-screen status immediately (always reflects real current data)
        ...
        // FIX 3: Only send a notification if threshold is breached AND cooldown has elapsed.
        // This ensures the notification body matches exactly what the app is displaying,
        // because we build the message from the same weatherData values used for the status above.
        if (isEnvTrigger) {
            const now = Date.now();
            if (now - lastNotificationRef.current > NOTIFICATION_COOLDOWN) {
                lastNotificationRef.current = now;
                const body = isPhysioTrigger
                    ? `High sweat risk! Temp: ${weatherData.temperature.toFixed(1)}°C, Humidity: ${weatherData.humidity.toFixed(0)}%, UV: ${weatherData.uvIndex.toFixed(1)}`
                    : `Climate alert! Temp: ${weatherData.temperature.toFixed(1)}°C, Humidity: ${weatherData.humidity.toFixed(0)}%, UV: ${weatherData.uvIndex.toFixed(1)}`;
                sendNotification('Sweat Smart Alert', body);
            }
        }
