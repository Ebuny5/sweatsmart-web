...
import { Target, TrendingUp, ExternalLink, Thermometer, Heart, Activity, Download } from "lucide-react";
...
const PersonalizedInsights: React.FC<PersonalizedInsightsProps> = ({ episodes }) => {
  ...
  const handleExportAllInsights = async () => {
    try {
      const pdf = new jsPDF();
      ...
      pdf.text('SweatSmart Comprehensive Clinical Report', margin, yPosition);
      yPosition += 15;
      ...
      yPosition += 8;
      pdf.text(`Total Episodes Analyzed: ${episodes.length}`, margin, yPosition);
      yPosition += 8;
      pdf.text(`Insights Generated: ${deduplicatedInsights.length}`, margin, yPosition);
      yPosition += 20;

      // Process each insight
      deduplicatedInsights.forEach((insight, index) => {
        ...
        pdf.setFontSize(10);
        pdf.setFont('helvetica', 'normal');
        pdf.text(`Confidence: ${insight.confidence} | Evidence: ${insight.evidenceTag}`, margin, yPosition);
        yPosition += 8;
        ...
        yPosition += 6;
        pdf.setFont('helvetica', 'normal');
        const splitSummary = pdf.splitTextToSize(insight.summary, pageWidth - 2 * margin);
        ...
        pdf.setFont('helvetica', 'bold');
        pdf.text('Immediate Action:', margin, yPosition);
        yPosition += 6;
        pdf.setFont('helvetica', 'normal');
        const splitImmediate = pdf.splitTextToSize(insight.actions.immediate, pageWidth - 2 * margin);
        ...
        pdf.setFont('helvetica', 'bold');
        pdf.text('Self-Management:', margin, yPosition);
        yPosition += 6;
        pdf.setFont('helvetica', 'normal');
        const splitSelfManagement = pdf.splitTextToSize(insight.actions.selfManagement, pageWidth - 2 * margin);
      ...
      pdf.text('This comprehensive report is for clinical reference only. Generated by SweatSmart App.', margin, yPosition);
      pdf.text('Patient should be evaluated clinically before any treatment decisions.', margin, yPosition + 10);

      // Save
      const filename = `sweatsmart-comprehensive-report-${format(new Date(), 'yyyy-MM-dd')}.pdf`;
      pdf.save(filename);

      toast({
        title: "Comprehensive report exported",
        description: `Clinical report with ${deduplicatedInsights.length} insights saved as ${filename}`,
      });
    } catch (error) {
      console.error('Error generating comprehensive clinical report:', error);
      toast({
        title: "Export failed",
          ...
          <Card key={insight.id} className={`border-l-4 ${insight.borderColor} ${insight.bgColor}`}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-2 border-t space-y-2">
                ...
                <Button variant="outline" size="sm" className="text-xs" onClick={handleExportAllInsights}>
