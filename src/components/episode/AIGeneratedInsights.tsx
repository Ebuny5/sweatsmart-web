...
const AIGeneratedInsights: React.FC<AIInsightsProps> = ({ insights }) => {
  ...
  const findBestVoice = (voices: SpeechSynthesisVoice[]) => {
    const preferred = [
      ...
      'aria', 'zira', 'hazel', 'susan', 'jenny',
    ];
    const english = voices.filter(v => v.lang.startsWith('en'));
    return (
      english.find(v => preferred.some(k => v.name.toLowerCase().includes(k))) ||
      english[0] ||
      voices[0] ||
      null
    );
  ...
  // ── Core speak function — works on Android Chrome ────────────────────────
  // Android Chrome requires:
  // 1. AudioContext to be resumed (unlocks audio engine)
  // 2. speechSynthesis.cancel() before any new speak
  // 3. A small delay after cancel
  // 4. A heartbeat to prevent Android from pausing long speech
  const speakText = (text: string) => {
    ...
    try {
      ...
      if (AudioCtx) {
        ...
        // Play a completely silent buffer — this unlocks the audio engine
      ...
      // AudioContext failed — proceed anyway
    }

    // Step 2: Cancel any existing speech and wait
    synth.cancel();
    ...
    setTimeout(() => {
      ...
      const performSpeak = () => {
        ...
        utterance.onstart = () => {
          ...
          // Step 3: Heartbeat — Android Chrome pauses speechSynthesis after ~1 min
          // This keeps it alive by resuming if paused
          if (heartbeatRef.current) clearInterval(heartbeatRef.current);
        ...
        // Android Chrome sometimes silently drops speak() if called too fast
        // Double check after 500ms and retry once if needed
        setTimeout(() => {
      ...
      // Load voices — on mobile they may not be ready immediately
      if (synth.getVoices().length > 0) {
        performSpeak();
      } else {
        synth.addEventListener('voiceschanged', performSpeak, { once: true });
        // Fallback if voiceschanged never fires (some browsers)
        setTimeout(() => {
          if (!synth.speaking) performSpeak();
        }, 1000);
      }
    }, 200); // 200ms delay after cancel — required on Android
  };
