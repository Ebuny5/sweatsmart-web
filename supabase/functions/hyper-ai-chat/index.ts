...
function generateWarriorReport(analytics: any, userName: string): string {
  ...
  return `SWEATSMART CLINICAL SUMMARY
Generated by Hidro Ally | ${new Date().toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
Patient: ${userName}
Report ID: SS-${userName.slice(0, 3).toUpperCase()}-${Date.now()}
...
Clinical Status: ${hdssInterpretation}
Monitoring Trend: ${trend}

PRESENTING COMPLAINT
The patient presents with primary focal hyperhidrosis. Over the documented period, the mean HDSS severity score is ${avgSeverity}.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
This report was generated by Hidro Ally, SweatSmart's AI clinical intelligence system.
It is intended to support — not replace — clinical consultation.
CONFIDENTIAL — Intended for patient and treating clinician only.
sweatsmart.guru`;
}
...
════════════════════════════════════
When a user uploads a photo — your first job is to IDENTIFY WHAT BODY PART IS IN THE IMAGE before analysing it. Look carefully for anatomical features (Palm vs Sole, Fingers vs Toes). Look for moisture, maceration, or swelling (Vasodilation-Edema Link).

FOR CLINICAL DOCUMENTS (hospital reports, lab results, prescriptions):
• Summarise key findings in plain language.
• Highlight anything relevant to hyperhidrosis.
