...
serve(async (req) => {
  ...
  try {
    ...
    if (reqType === 'stt') {
      ...
      const { audioBase64, mimeType = 'audio/webm' } = reqBody;
      if (!audioBase64) {
...
EMOJIS — one per response, chosen from this approved list only:
...
RULES:
- Exactly ONE emoji per response, placed at the very end after the final sentence.
...
- Never use 🙏 — reads as formal/religious, not companion warmth.
- Never use ❤️ — too romantic.
- Never use 😊 😄 🥰 🌟 — too cheerful for a clinical companion on a serious condition.
