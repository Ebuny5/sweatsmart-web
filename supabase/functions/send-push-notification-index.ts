...
async function getAppServer() {
  ...
  if (publicKeyBytes.length === 91) {
    publicKeyBytes = publicKeyBytes.slice(26, 91);
  }
...
// FIXED: Risk based on REAL temperature, not heat index
// Prevents fake "Extreme Risk" at 22°C with high humidity
// UV always capped at 11
function calculateSweatRisk(temp: number, humidity: number, uv: number) {
  const safeUV = Math.min(11, uv);

  // Extreme: real temp >= 35°C OR UV >= 11
  if (temp >= 35 || safeUV >= 11) return 'extreme';

  // High: real temp >= 32°C
  if (temp >= 32) return 'high';

  // Moderate: real temp >= 28°C AND humidity >= 80%
  if (temp >= 28 && humidity >= 80) return 'moderate';

  // Moderate: real temp >= 28°C
...
serve(async (req) => {
  ...
  try {
    ...
    // Public: Return VAPID public key — no auth needed
        ...
        try {
          ...
          if (!isNight) {
            try {
              ...
              uv = Math.min(11, uvData.value || 0); // Always cap UV at 11
          ...
          // FIXED: uses real temp, not heat index
          ...
          let body: string;
          if (risk === 'extreme') {
            title = '🚨 SweatSmart: Extreme Heat Risk';
            body = `Temp ${temp.toFixed(0)}°C, humidity ${humidity}% — stay indoors with AC! UV: ${uv.toFixed(0)}`;
          } else if (risk === 'high') {
            title = '⚠️ SweatSmart: High Heat Risk';
            body = `Temp ${temp.toFixed(0)}°C, humidity ${humidity}% — use cooling aids and stay hydrated.`;
          } else {
            title = '💧 SweatSmart: Moderate Heat Risk';
            body = `Temp ${temp.toFixed(0)}°C, humidity ${humidity}% — prepare cooling aids.`;
          }
